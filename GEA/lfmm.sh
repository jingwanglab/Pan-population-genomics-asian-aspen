#!/bin/bash
set -e
indir="/xxx/LEA3/input_file/pd222_phase"
outdir="/xxx/LEA3/input_file/LFMM"
envdir="/xxx/LEA3/input_file/env"
pop="/xxx/LEA3/pop.txt"
sh_file="/xxx/LFMM_onebio_new.sh"
script_dir="/xxx/LFMM_onebio_10.R"
#create Rscript_dir
...
do
	...
	do	
		...
		Rscript=${dir##*/}$bio.R
		lfmm=${dir##*/}.pd222_phase_$bio.lfmm
...
project <- lfmm(\"$lfmm\",\"$env\",CPU=6,K = 2,repetitions = 5, project = \"new\")
p = lfmm.pvalues(project, K = 2,  d = 1, run = 1)
pvalues = p\$pvalues
write.csv(pvalues,\"$file1\")
p = lfmm.pvalues(project, K = 2,  d = 1, run = 2)
pvalues = p\$pvalues
write.csv(pvalues,\"$file2\")
p = lfmm.pvalues(project, K = 2,  d = 1, run = 3)
pvalues = p\$pvalues
write.csv(pvalues,\"$file3\")
p = lfmm.pvalues(project, K = 2,  d = 1, run = 4)
pvalues = p\$pvalues
write.csv(pvalues,\"$file4\")
p = lfmm.pvalues(project, K = 2,  d = 1, run = 5)
pvalues = p\$pvalues
