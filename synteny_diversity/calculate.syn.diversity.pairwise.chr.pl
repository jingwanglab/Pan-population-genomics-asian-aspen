...
#===============================================================================
#
#         FILE: calculate.syn.diversity.pairwise.chr.pl
#
#        USAGE: ./calculate.syn.diversity.pairwise.chr.pl  
#
#  DESCRIPTION: 
#
#      OPTIONS: ---
...
#      VERSION: 1.0
#      CREATED: 02/19/2019 10:08:22 AM
#     REVISION: ---
#===============================================================================
...
my ($allSyn,$alnDir,$lenFileDir,$sampe1,$sampe2,$chrom,$out)=@ARGV;


my @accs=("ZSP191aL","ZSP192L","ZL2020-SXHZ","ZSP194L","ZSP196L");
#my @accs=("An-1","C24","Col");
## get chrom length
my %len;
foreach my $acc (@accs) {
  #next if ($acc ne $sampe1 or $acc ne $sampe2);
  print "#leng $acc\n";
...
my $ref = "ZL2020-SXHZ";
open IN,"grep $chrom $allSyn|"; ## 1 based coord,
my %coord;
my $ch;
while (<IN>) {
  chomp;
...
my %aln; my $rIx=2;
my %alnRef; my %trans; ## used to transfer the coordinate
...
foreach my $i (0..$#accs-1) {
  foreach my $j ($i+1..$#accs) {  	
  	 my $acc1 = $accs[$i];
  	 my $acc2 = $accs[$j];
  	 next if ($acc1 ne $sampe1 or $acc2 ne $sampe2);
  	 print "#check syn: $acc1\t$acc2\n";
  	 
  	 $comNum+=1;  	 
  	 #next;
  	 if ($acc1 eq $ref) {  	 	  	
  	 	my $alnFile = "$alnDir/$acc1/$acc2/out_m_i90_l100.$chrom.aligns";
  	    my $blkFile = "$alnDir/$acc1/$acc2/$acc2.wga.block.txt";  	  	  	 	
  	 	my $blk = getBlk($blkFile,$chrom);
  	 	my ($alnRef,$trans) = getAlnRef($alnFile,$blk);
  	 	$alnRef{$acc1}{$acc2} = $alnRef;
  	 	$trans{$acc1}{$acc2} = $trans;
  	 }elsif ($acc2 eq $ref) {
  	 	my $alnFile = "$alnDir/$acc2/$acc1/out_m_i90_l100.$chrom.aligns";
  	    ...
  	    my $blkFile = "$alnDir/$acc1/$acc2/$acc2.wga.block.txt";  	  	  	 	
  	 	...
  	 	##for coordinate translation of acc1
  	 	$alnFile = "$alnDir/$ref/$acc1/out_m_i90_l100.$chrom.aligns";
  	    $blkFile = "$alnDir/$ref/$acc1/$acc1.wga.block.txt";  	  	  	 	
  	 	$blk = getBlk($blkFile,$chrom);
  	 	  	 	
  	 	my ($alnRef,$trans) = getAlnRef($alnFile,$blk);
  	 	$alnRef{$ref}{$acc1} = $alnRef;
  	 	$trans{$ref}{$acc1} = $trans;
  	 	
  	 	##for coordinate translation of acc2
  	 	$alnFile = "$alnDir/$ref/$acc2/out_m_i90_l100.$chrom.aligns";
  	    $blkFile = "$alnDir/$ref/$acc2/$acc2.wga.block.txt";  	  	  	 	
  	 	$blk = getBlk($blkFile,$chrom);
  	 	  	 	
  	 	($alnRef,$trans) = getAlnRef($alnFile,$blk);
  	 	$alnRef{$ref}{$acc2} = $alnRef;
  	 	$trans{$ref}{$acc2} = $trans;
...
foreach my $chr (sort keys %coord) {  
  ...
  foreach my $i (1..$#syns) {
   	 ...
   	 foreach my $k ($j+1..$#accs) {
   	    ...
   	    if ($acc1 eq $ref) {
   	      ...
   	      foreach my $pos ($start2..$end2) {   	      	
   	       ...
   	       foreach my $pos ($refSt..$refEnd) {
   	       	  #print "$pos\n";exit;
   	       	  if (!$trans{$ref}{$acc1}{$chr}) {
   	       	  	print "##no translation infor. for $acc1 $chr \n";
   	       	  	exit;
   	       	  }
   	       	  if (! $trans{$ref}{$acc1}{$chr}{$pos} and ! $trans{$ref}{$acc2}{$chr}{$pos}) { ## both del
   	       	  	...
   	       	  	foreach my $pos1 (@pos1) {
   	       	  		...
   	       	  		foreach my $pos2 (@pos2) {
   	       	  		   ...
   	       	  		   if (!$aln{$acc1}{$acc2}{$chr}) {
   	       	  		   	  print "##no alignment for $acc1 $acc2 at $chr \n";
   	       	  			  exit;
   	       	  		   }
   	       	  		   if ($aln{$acc1}{$acc2}{$chr}{$pos1} and $aln{$acc2}{$acc1}{$chr}{$pos2}  ) {
   	       	  		   	  $syn = 1;
   	       	  		   	  #print "yy\t$pos\t$pos1\t$pos2\n";
   	       	  		   }	
   	       	  		}
   	       	  	}
   	       	  	if ($syn == 1) {
   ...
   next if ($i==$#syns);
   foreach my $pos ($refSt..$refEnd) {   	       	     	
   	  print OUT "$chr\t$pos\t$comNum\t0\t0.000\n";
   }       
 }     
...
sub getAln {
   ...
   while (<IN>) {
   	  chomp;
   	  if (/Align/) {
   	  	#last if ($chr and  $chr ne (split /\s+/,$_)[3] );
   	  	($chr)=(split /\s+/,$_)[3];
   	  	$chr=~s/chr/Chr/g;
   	  	print "$chr\n";
