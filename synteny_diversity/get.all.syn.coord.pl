...
my @accs=("ZSP191aL","ZSP192L","ZSP194L","ZSP196L");
my %syn;
foreach my $acc (@accs) {
   print "get aln $acc\n";
...
sub getPairSYN {
   ...
   while (<IN>) {
   	  ...
   	  my @t = (split /\t/);
   	  #last if ($t[0] eq "Chr2");
   	  if ($t[1] >= $t[10] and $t[1] <= $t[11]) {
   	  	my $pairStart = $aln{"$t[9]\t$t[10]\t$t[11]"}{$t[1]};
   	  	if (!$pairStart) {
   	  ...
   	  if ($t[2] >= $t[10] and $t[2] <= $t[11]) {
   	  	my $pairEnd = $aln{"$t[9]\t$t[10]\t$t[11]"}{$t[2]};   	  	
   	  	if (!$pairEnd) {
   	  	  print "no pair end $_\n";
   	  	}
